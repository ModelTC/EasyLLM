...
class BaichuanLayer(torch.nn.Module):
    def __init__(self, config: BaichuanConfig):
        ...
        if not getattr(config, "_flash_attn_2_enabled", False):
            ...
            self.self_attn = FlashAttention(config=config, pack_qkv=True, rotary_emb=False)
        ...
        NormLayer = FlashRMSNorm if getattr(config, "_flash_norm_2_enabled", False) else RMSNorm  # noqa
        self.input_layernorm = NormLayer(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = NormLayer(
    ...
    def __init__(self, config, *model_args, **model_kwargs):
        ...
        if hasattr(config, "quantization_config") and isinstance(config.quantization_config,
            ...
            try:
                ...
                raise ImportError("Needs quantize_offline to run quantize.")
        ...
        **kwargs,
        ...
        if hasattr(config, "quantization_config") and config.quantization_config['load_in_4bit']:
            try:
                ...
                raise ImportError("Needs import model weight init func to run quantize.")
            # Instantiate model.
