...
from llm.plugins.internvl.models.mg_models import get_layer_info
...
class HuskyBaseRunner(BaseRunner):
    ...
    def build_model(self):
        ...
        for name, child in self.model.named_children():
            ...
            elif freeze_vit and int(name) < _HUSKY_MODELS[model_type]["num_intern_layers"] + 2:  # freeze vit
                self._freeze_params(child, is_eval=True)
            elif freeze_llm and int(name) >= _HUSKY_MODELS[model_type]["num_intern_layers"] + 3 and int(name) < _HUSKY_MODELS[model_type]["num_intern_layers"] + _HUSKY_MODELS[model_type]["num_layers"] + 7:  # noqa
                self._freeze_params(child, is_eval=True)
            elif freeze_mlp and int(name) == _HUSKY_MODELS[model_type]["num_intern_layers"] + 2:
                self._freeze_params(child)
        for name, child in self.model.named_children():
            if name == "tied_modules":
                continue
            elif unfreeze_lm_head and int(name) == _HUSKY_MODELS[model_type]["num_intern_layers"] + _HUSKY_MODELS[model_type]["num_layers"] + 6:
...
def main():
    ...
    if args.inference:
        ...
        runner = HuskyBaseRunner(args, cfg, training=False, base_type='infer')
        runner.generate()
    else:
        runner = HuskyBaseRunner(args, cfg, training=True, base_type='train')
        runner.train()
