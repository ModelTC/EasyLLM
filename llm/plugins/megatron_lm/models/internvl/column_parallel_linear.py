...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def forward(
        ...
        if sequence_parallel:
            ...
            all_gather_buffer = parallel_state.get_global_memory_buffer().get_tensor(dim_size, input.dtype, "dist_env")
            torch.distributed._all_gather_base(
                all_gather_buffer, input, group=parallel_state.get_tensor_model_parallel_group()
            )
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.sequence_parallel:
            ...
            handle = torch.distributed._all_gather_base(
                all_gather_buffer, input, group=parallel_state.get_tensor_model_parallel_group(), async_op=True
            )
        ...
        if ctx.sequence_parallel:
            ...
            handle = torch.distributed._reduce_scatter_base(
                sub_grad_input, grad_input, group=parallel_state.get_tensor_model_parallel_group(), async_op=True
            )
            # Here we rely on CUDA_DEVICE_MAX_CONNECTIONS=1 to ensure that the
