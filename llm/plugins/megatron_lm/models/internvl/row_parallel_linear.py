...
class RowParallelLinear(torch.nn.Module):
    ...
    def forward(self, input_):
        ...
        if self.input_is_parallel:
            ...
            input_parallel = tensor_parallel.scatter_to_tensor_model_parallel_region(input_)
        # Matrix multiply.
        output_parallel = F.linear(input_parallel, self.weight)
        # All-reduce across all the partitions.

        if self.sequence_parallel:
            output_ = tensor_parallel.reduce_scatter_to_sequence_parallel_region(output_parallel)
        else:
            output_ = tensor_parallel.reduce_from_tensor_model_parallel_region(output_parallel)
