...
from megatron.core.tensor_parallel.layers import ColumnParallelLinear as mgCore_ColumnParallelLinear
from megatron.core.tensor_parallel.layers import RowParallelLinear as mgCore_RowParallelLinear
...
def hf_to_megatron_internvl(dt, model, is_pack=False):
    ...
    #     if 'vision_embedding' in key:
    #         dst_key = key.replace('vision_embedding', 'vision_model.embeddings')
    #         output_dt[key] = dt[dst_key]
        
    #     elif 'vision_transformer_layer' in key:
    #         layer_id = int(key.split('.')[0].split('_')[1])
    #         param_name = '.'.join(key.split('.')[1:])
            
    #         param_name = param_name.replace('projection', 'proj')
    #         dst_key = f"vision_model.encoder.layers.{layer_id}.{param_name}"
    #         output_dt[key] = dt[dst_key]

    #     elif 'vision_extract_feat' in key:
    #         if 'mlp1_norm.weight' in key:
    #             output_dt[key] = dt['mlp1_norm.weight']
    #         elif 'mlp1_norm.bias' in key:
    #             output_dt[key] = dt['mlp1.0.bias']
    #         elif 'mlp1_fc1.weight' in key:
    #             output_dt[key] = dt['mlp1.1.weight']
    #         elif 'mlp1_fc1.bias' in key:
    #             output_dt[key] = dt['mlp1.1.bias']
    #         elif 'mlp1_fc2.weight' in key:
    #             output_dt[key] = dt['mlp1.3.weight']
    #         elif 'mlp1_fc2.bias' in key:
    #             output_dt[key] = dt['mlp1.3.bias']
    #         else:
    #             logger.info(f"unuse keys {key}")
        
    #     elif 'language_transformer_layer' in key:
    #         layer_id = int(key.split('.')[0].split('_')[1])
    #         if 'linear_qkv' in key:
    #             dst_key = f'language_model.model.layers.{layer_id}.attention.wqkv.weight'
    #             qkv = dt[dst_key]
    #             qkv = qkv.reshape(-1, gs + 2, head_dim, hid_size)
    #             q = qkv[:, :gs, ...].reshape(-1, hid_size)
    #             k = qkv[:, -2, ...].reshape(-1, hid_size)
    ...
    #             dst_key = f'language_model.model.layers.{layer_id}.attention.wo.weight'
    #             output_dt[key] = dt[dst_key]
    #         elif 'mlp.linear_fc1' in key:
    #             w1 = dt[f'language_model.model.layers.{layer_id}.feed_forward.w1.weight']
    #             w3 = dt[f'language_model.model.layers.{layer_id}.feed_forward.w3.weight']
    #             output_dt[f'{layer_name}.mlp.linear_fc1.weight'] = torch.cat(
    #                     [w1_weight, w3_weight], 0).contiguous().clone()
                
    for key in dt.keys():
        if torch.distributed.get_rank() == 0:
            print(f'load_func: {key}')

        if 'vision_model.embeddings.patch_embedding.weight' in key:
            output_dt['vision_embedding.patch_embedding.weight'] = dt[key]
        elif 'vision_model.embeddings.patch_embedding.bias' in key:
            output_dt['vision_embedding.patch_embedding.bias'] = dt[key]
        elif 'vision_model.embeddings.position_embedding' in key:
            output_dt['vision_embedding.position_embedding'] = dt[key]
        elif 'vision_model.embeddings.class_embedding' in key:
            output_dt['vision_embedding.class_embedding'] = dt[key]

        elif 'vision_model.encoder.layers' in key:
            layer_id = int(key.split('.')[3])
            layer_name = f"vision_transformer_layer_{layer_id}"
            if 'norm1.weight' in key:
                output_dt[f'{layer_name}.norm1.weight'] = dt[key]
            elif 'norm1.bias' in key:
                output_dt[f'{layer_name}.norm1.bias'] = dt[key]
            elif 'norm2.weight' in key:
                output_dt[f'{layer_name}.norm2.weight'] = dt[key]
            elif 'norm2.bias' in key:
                output_dt[f'{layer_name}.norm2.bias'] = dt[key]
            elif 'attn.qkv.weight' in key:
                output_dt[f'{layer_name}.attn.qkv.weight'] = dt[key]
            elif 'attn.proj.weight' in key:
                output_dt[f'{layer_name}.attn.projection.weight'] = dt[key]
            elif 'attn.proj.bias' in key:
                output_dt[f'{layer_name}.attn.projection.bias'] = dt[key]
            elif 'mlp.fc1.weight' in key:
                output_dt[f'{layer_name}.mlp.fc1.weight'] = dt[key]
            elif 'mlp.fc1.bias' in key:
                output_dt[f'{layer_name}.mlp.fc1.bias'] = dt[key]
            elif 'mlp.fc2.weight' in key:
                output_dt[f'{layer_name}.mlp.fc2.weight'] = dt[key]
            elif 'mlp.fc2.bias' in key:
                output_dt[f'{layer_name}.mlp.fc2.bias'] = dt[key]
            elif 'ls1' in key:
                output_dt[f'{layer_name}.ls1'] = dt[key]
            elif 'ls2' in key:
                output_dt[f'{layer_name}.ls2'] = dt[key]
            elif 'q_norm' in key:
                output_dt[f'{layer_name}.attn.q_norm.weight'] = dt[key]
            elif 'k_norm' in key:
                output_dt[f'{layer_name}.attn.k_norm.weight'] = dt[key]
            else:
                logger.info(f"unuse keys {key}")

        elif key.startswith('mlp1'):
            # layer_id = vit_num_layers + 2
            layer_name = f"vision_extract_feat"
            if 'mlp1.0.weight' in key:
                output_dt[f'{layer_name}.mlp1_norm.weight'] = dt[key]
            elif 'mlp1.0.bias' in key:
                output_dt[f'{layer_name}.mlp1_norm.bias'] = dt[key]
            elif 'mlp1.1.weight' in key:
                output_dt[f'{layer_name}.mlp1_fc1.weight'] = dt[key]
            elif 'mlp1.1.bias' in key:
                output_dt[f'{layer_name}.mlp1_fc1.bias'] = dt[key]
            elif 'mlp1.3.weight' in key:
                output_dt[f'{layer_name}.mlp1_fc2.weight'] = dt[key]
            elif 'mlp1.3.bias' in key:
                output_dt[f'{layer_name}.mlp1_fc2.bias'] = dt[key]
            else:
                logger.info(f"unuse keys {key}")
            ...
            n_kv_heads = model.language_transformer_config.num_query_groups
            n_heads =  model.language_transformer_config.num_attention_heads
            hid_size = model.language_transformer_config.hidden_size

            gs = n_heads // n_kv_heads
            head_dim = hid_size // n_heads
            if 'tok_embeddings' in key:
                # layer_id = vit_num_layers + 3
                output_dt[f'word_embedding.word_embeddings.weight'] = dt[key]
            elif 'model.norm' in key:
                output_dt[f'final_layernorm.weight'] = dt[key]
            elif 'output' in key:
                output_dt[f'lm_head.weight'] = dt[key]
            elif 'layers' in key:
                # layer_id = int(key.split('.')[3]) + vit_num_layers + 4
                layer_id = int(key.split('.')[3])
                layer_name = f"language_transformer_layer_{layer_id}"
                if 'attention.wqkv' in key:
                    qkv = dt[key]
                    qkv = qkv.reshape(-1, gs + 2, head_dim, hid_size)
                    q = qkv[:, :gs, ...].reshape(-1, hid_size)
                    ...
                    v = qkv[:, -1, ...].reshape(-1, hid_size)
                    if is_pack:
                        output_dt[f"{layer_name}.self_attention.linear_qkv.weight"] = [q, k, v]
                    else:
                        output_dt[f"{layer_name}.self_attention.q_proj.weight"] = q
                        output_dt[f"{layer_name}.self_attention.k_proj.weight"] = k
                        output_dt[f"{layer_name}.self_attention.v_proj.weight"] = v
                elif 'attention.wo' in key:
                    output_dt[f'{layer_name}.self_attention.linear_proj.weight'] = dt[key]
                # elif 'feed_forward.w1' in key:
                #     output_dt[f'{layer_name}.mlp.gate_proj.weight'] = dt[key]
                # elif 'feed_forward.w2' in key:
                #     output_dt[f'{layer_name}.mlp.down_proj.weight'] = dt[key]
                # elif 'feed_forward.w3' in key:
                #     output_dt[f'{layer_name}.mlp.up_proj.weight'] = dt[key]

                elif 'feed_forward.w1' in key:
                    w1_weight = dt[key]
                    w3_weight = dt[key.replace('w1', 'w3')]
                    # output_dt[f'{layer_name}.mlp.linear_fc1.weight'] = torch.cat(
                    #         [w1_weight, w3_weight], 0).contiguous().clone()
                    output_dt[f'{layer_name}.mlp.linear_fc1.weight'] = [w1_weight, w3_weight]
                elif 'feed_forward.w2' in key:
                    output_dt[f'{layer_name}.mlp.linear_fc2.weight'] = dt[key]
                elif 'feed_forward.w3' in key:
                    pass

                elif 'ffn_norm.weight' in key:
                    output_dt[f'{layer_name}.pre_mlp_layernorm.weight'] = dt[key]
                elif 'attention_norm.weight' in key:
                    output_dt[f'{layer_name}.input_layernorm.weight'] = dt[key]
                else:
                    logger.info(f"unuse keys {key}")
            else:
                logger.info(f"unuse keys {key}")
...
from llm.plugins.megatron_lm.utils.megatron_checkpointing import permute_qkv_weight
def load_func(dt, tp_rank, tp_world_size, model, num_layers, lora_mode, pretrain_type, init_set, vit_layers=None, load_mlp1=True):
    ...
    for name in dt.keys():
        # if torch.distributed.get_rank() == 0 and name == "vision_transformer_layer_2.norm1.weight":
        #     import pdb;pdb.set_trace()
        param, module = get_custom_module_param(name, model)
        # torch.distributed.barrier()
        if module is None:
            continue
        print(f'rank:{torch.distributed.get_rank()}, name:{name}, module:{type(module)}')
        # if 'mlp.linear_fc1.weight' in name:
        #     dt[name] = torch.cat(dt[name], 0)
        slice_ = dt[name]
        if isinstance(module, VocabParallelEmbedding):  # noqa
            start, stop = get_start_end(param.shape[0], tp_world_size, tp_rank)
            tensor = slice_[start:stop]
        elif isinstance(module, ColumnParallelLinear) or isinstance(module, TEColumnParallelLinear) or isinstance(module, mgCore_ColumnParallelLinear):
            if getattr(module, 'num_attention_heads', None):
            ...
            # elif is_pack and isinstance(slice_, list):
                ...
                tensor = torch.cat([q_slice_, k_slice_, v_slice_], dim=0)
                # permute tensor
                num_kv_heads = model.language_transformer_config.num_query_groups
                n_heads =  model.language_transformer_config.num_attention_heads
                hidden_size = model.language_transformer_config.hidden_size
                tensor = permute_qkv_weight(tensor, (n_heads, hidden_size, tp_world_size, num_kv_heads))
                ...
                q_slice_ = slice_[0][q_start:q_stop]
                k_slice_ = slice_[1][k_start:k_stop]
                tensor = torch.cat([q_slice_, k_slice_], dim=0)
        ...
        elif isinstance(module, RowParallelLinear) or isinstance(module, TERowParallelLinear) or isinstance(module, mgCore_RowParallelLinear):
            if len(param.shape) == 1:  # for bias
                tensor = slice_
            elif getattr(module, 'num_attention_heads', None):
                num_heads = module.num_attention_heads
...
def load_llama_from_hf_format(load_dirs,
    ...
    for idx in range(len(load_dirs)):
        ...
        from safetensors.torch import load_file as safe_load_file
        print('load file')
        dict_list = [safe_load_file(filename) for filename in filenames]
        print('merge file')
        merge_dict = {}
        for d in dict_list:
            merge_dict.update(d)
        print('load func')
        load_func(merge_dict, tp_rank, tp_world_size, model, num_layers, lora_mode, pretrain_type, init_set, vit_layers=vit_layers, load_mlp1=load_mlp1)
...
def load_ckpt_pretrained(cfg_loader, model, optimizer):
    if cfg_loader.get("debug", False):
        ...
        if cfg_loader['load_mode'] == 'huggingface':
            worker = cfg_loader.get('worker', 1)
